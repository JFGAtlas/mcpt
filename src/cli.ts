...
Options:
  -u, --update-snapshots   Accept changed snapshots
...
async function cmdRun(args: string[]): Promise<number> {
  ...
  if (files.length === 0) {
    ...
    if (files.length === 0) {
      console.error(
        `no test files found — expected mcpt.yaml or *.mcpt.yaml in the current directory.\n` +
          `Generate one from a running server:  mcpt init -- <command to start server>`,
      );
      return 2;
    }
  }
...
async function cmdInit(args: string[]): Promise<number> {
  ...
  if (serverArgs.length === 0) {
    console.error(`usage: mcpt init [-o file] -- <command to start server>`);
    return 2;
  }
...
async function cmdList(args: string[]): Promise<number> {
  ...
  if (serverArgs.length === 0) {
    console.error(`usage: mcpt list -- <command to start server>`);
    return 2;
  }
